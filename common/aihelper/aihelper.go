...
// 同步生成
func (a *AIHelper) GenerateResponse(userName string, ctx context.Context, userQuestion string, options ChatOptions) (*model.Message, error) {
	...
	//调用模型生成回复
	schemaMsg, err := a.model.GenerateResponse(ctx, messages, options)
	if err != nil {
		return nil, err
	}
...
// 流式生成
func (a *AIHelper) StreamResponse(userName string, ctx context.Context, cb StreamCallback, userQuestion string, options ChatOptions) (*model.Message, error) {
	...
	content, err := a.model.StreamResponse(ctx, messages, cb, options)
	if err != nil {
		return nil, err
	}
	//转化成model.Message
